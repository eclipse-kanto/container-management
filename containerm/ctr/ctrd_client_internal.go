...
func (ctrdClient *containerdClient) getImage(ctx context.Context, imageInfo types.Image) (containerd.Image, error) {
	...
	if err = ctrdClient.verifyImage(ctx, ctrdImage, false, verificationKeys); err != nil {
		return nil, err
	}
...
func (ctrdClient *containerdClient) pullImage(ctx context.Context, imageInfo types.Image) (containerd.Image, error) {
	...
	if err != nil {
		...
		if errdefs.IsNotFound(err) {
			...
			if verifyErr := ctrdClient.verifyImage(ctx, ctrdImage, true, vk); verifyErr != nil {
...
func (ctrdClient *containerdClient) verifyImage(ctx context.Context, image containerd.Image, pull bool, verificationKeys []*verificationKey) error {
	...
	if len(verificationKeys) == 0 {
		...
		log.Debug(skipVerificationMessage, imageName, "no keys are provided")
		return nil
	}
	...
	if err != nil {
		// if the image is not present locally - either pull it(ctr create), or skip verification(ctr start)
		if errdefs.IsNotFound(err) {
			if !pull {
				log.DebugErr(err, skipVerificationMessage, "no signature is found locally", imageName)
				return nil
			}
			...
			if err != nil {
				if strings.Contains(err.Error(), "not found") {
					...
					log.WarnErr(err, skipVerificationMessage, "no signature", imageName)
					return nil
				}
				return err
			}
...
func (ctrdClient *containerdClient) initImagesExpiryManagement(ctx context.Context) error {
	...
	for _, image := range images {
		...
		if signatureRegex.MatchString(imageRef) {
			// skip signatures they will be removed together with the corresponding image
			continue
		}

		watchErr := ctrdClient.manageImageExpiry(ctx, image)
		if watchErr != nil {
			log.DebugErr(watchErr, "error while initializing expiry management for image = %s", imageRef)
			continue
		}
		log.Debug("successfully managed expiry for image = %s", imageRef)
	}
	return nil
}
...
func (ctrdClient *containerdClient) removeUnusedImage(ctx context.Context, image containerd.Image) error {
	...
	if delSigErr := ctrdClient.spi.DeleteImage(ctx, signatureRef); delSigErr != nil {
		...
		log.Debug("deleted signature of unused image = %s", imgRef)
	}
