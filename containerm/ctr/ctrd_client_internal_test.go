...
func TestClientInternalDeleteImage(t *testing.T) {
	testCases := map[string]struct {
		mockExec func(*mocksCtrd.MockcontainerdSpi) error
	}{
		"test_delete_image_error": {
			mockExec: func(spiMock *mocksCtrd.MockcontainerdSpi) error {
				err := log.NewError("test error")
				spiMock.EXPECT().DeleteImage(gomock.Any(), testImageRef).Return(err)
				return err
			},
		},
		"test_delete_image_no_error": {
			mockExec: func(spiMock *mocksCtrd.MockcontainerdSpi) error {
				spiMock.EXPECT().DeleteImage(gomock.Any(), testImageRef).Return(nil)
				return nil
			},
		},
	}
	...
	for testCaseName, testCaseData := range testCases {
		t.Run(testCaseName, func(t *testing.T) {
			...
			expectedErr := testCaseData.mockExec(spiMock)
			actualErr := ctrdClient.deleteImage(context.TODO(), testImageRef)
