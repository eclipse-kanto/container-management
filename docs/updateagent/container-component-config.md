### Overview
This documentation page describes the list of all supported configuration properties for a container component. The properties are represented as a key/value pair in the `config` section for a
component, defined in the Desired State Specification for the domain.

### Domain Identifier
The domain identifier for the Containers Update Agent is `containers`.
